	...
	while(now_token) {
		...
		{	
			...
			else if(next_token==VAR_NAME) //satisfies variable name convention
			{
				...
				{
					super_error_check=1;
					printf("Line %d: ERROR: Expected a semi colon after variable name\n",yylineno);
					error_check=1;
				}
		...
		////////////////////////////////////////////////////////////
		else if (error_check==0&&now_token==CMT_START)// comment is declared 
		{

			while(now_token!=CMT_END)
				now_token=yylex();
			now_token=yylex();// comment ends here
		}
		if(error_check==0&&now_token==TYPE)
		{
			...
			if (next_token==UNKNOWN||next_token==COMMA)
			{
				...
				super_error_check=1;
				if (next_token==COMMA)
				printf("Line %d: ERROR: Variable Name is not following conventions\n",yylineno);
				else
				{
					next_token=yylex();
					if(next_token==LEFT_BRACKET)
			...
			{
				//printf("Line %d l8\n",yylineno );
				while(next_token!=SEMI_COLON&&next_token!=LEFT_CURLY&&next_token!=COMMA)
					next_token=yylex();
				if (next_token==COMMA)
					goto type_rec;
					//error_check=1;
